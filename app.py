...
# The reference clause library is optional: if chromadb or its local embedding model
# is unavailable in the deployment environment, the rest of the app still works.
try:
    from clause_library import search_similar_clauses
except Exception:
...
def render_clause(r):
    ...
    # Flagged clauses are compared against the reference corpus so the reviewer can
    # see how standard agreements phrase the same obligation.
    if r["level"] in ("RED", "YELLOW"):
        matches = search_similar_clauses(r["original_excerpt"], n_results=3)
        if matches:
            with st.expander(f"Compare with {len(matches)} reference clauses"):
                for m in matches:
                    st.markdown(
                        f"**{m['source']}** · similarity {m['similarity']:.2f}\n\n{m['text']}"
                    )
                    st.markdown("---")
